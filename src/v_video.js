let screens = []; // LENGTH: 5

let dirtybox = []; // LENGTH: 4 
...
function V_CopyRect (srcx, srcy, srcscrn, width, height, destx, desty, destscrn) { 
    ...
    if(RANGECHECK) {
        ...
        || destscrn>4)
        {
            console.error("Bad V_CopyRect");
        }
    }
    V_MarkRect (destx, desty, width, height); 
    
    // cannot use pointers, just get the index
    src = (SCREENWIDTH*SCREENHEIGHT*srcscrn)+SCREENWIDTH*srcy+srcx; 
    dest = (SCREENWIDTH*SCREENHEIGHT*destscrn)+SCREENWIDTH*desty+destx; 
...
function V_DrawBlock (x, y, scrn, width, height, src) { 
    ...
    dest = SCREENWIDTH*SCREENHEIGHT*scrn + y*SCREENWIDTH+x; 
